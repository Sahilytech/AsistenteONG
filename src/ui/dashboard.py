...
class DashboardFrame(ctk.CTkFrame):
    ...
    def run_search(self):
        ...
        try:
            results,online=search_official(query,self.memory,limit=8); self.search_status.configure(text=("Conectado: resultados guardados en memoria local." if online else "Sin conexión: mostrando memoria local."),text_color=COLORS["success"] if online else COLORS["warning"])
            if not results: ctk.CTkLabel(self.results_box,text="No hay resultados guardados para esta búsqueda.",font=FONTS["small"],text_color=COLORS["text_muted"]).pack(anchor="w",padx=10,pady=12); return
            for result in results:
                item=ctk.CTkFrame(self.results_box,fg_color=COLORS["surface"],corner_radius=10,border_width=1,border_color=COLORS["border"]); item.pack(fill="x",padx=3,pady=4); ctk.CTkLabel(item,text=result.title,font=FONTS["small_bold"],text_color=COLORS["primary"],anchor="w",justify="left",wraplength=650).pack(fill="x",padx=10,pady=(8,1)); ctk.CTkLabel(item,text=result.snippet,font=FONTS["tiny"],text_color=COLORS["text_muted"],anchor="w",justify="left",wraplength=650).pack(fill="x",padx=10,pady=(0,3)); ctk.CTkButton(item,text=result.domain,width=120,height=24,fg_color=COLORS["primary_soft"],hover_color=COLORS["primary"],text_color=COLORS["primary"],command=lambda u=result.url:webbrowser.open(u)).pack(anchor="e",padx=10,pady=(0,7))
