"""
Dashboard con estadísticas reales desde SQLite
"""
...
class CaseStats:
    """Estadísticas de casos desde DB."""

    def __init__(self):
        self.total_cases = 0
        self.by_urgency = {"Muy Alta": 0, "Alta": 0, "Media": 0, "Baja": 0}
        self.by_category = {}
        self.cases_today = 0
    ...
    def _load_from_db(self):
        ...
        try:
            ...
            self.total_cases = cursor.fetchone()[0]

            # Por urgencia
            cursor.execute("SELECT urgency, COUNT(*) FROM cases GROUP BY urgency")
            for row in cursor.fetchall():
                if row[0]:
                    self.by_urgency[row[0]] = row[1]

            # Casos de hoy
            today = datetime.now().strftime("%Y-%m-%d")
            cursor.execute("SELECT COUNT(*) FROM cases WHERE date(created_at) = ?", (today,))
            self.cases_today = cursor.fetchone()[0]

            # Casos de esta semana
            week_ago = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
            cursor.execute("SELECT COUNT(*) FROM cases WHERE date(created_at) >= ?", (week_ago,))
            self.cases_this_week = cursor.fetchone()[0]

            conn.close()
            logger.info(f"📊 Stats cargadas: {self.total_cases} casos")
...
class DashboardFrame(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ...
        self._create_metric(metrics_frame, "📋 Total", str(self.stats.total_cases), 0)

        # Métrica 2: Muy Altas
        self._create_metric(metrics_frame, "🔴 Muy Alta", str(self.stats.by_urgency["Muy Alta"]), 1)

        # Métrica 3: Hoy
        self._create_metric(metrics_frame, "📅 Hoy", str(self.stats.cases_today), 2)

        # Métrica 4: Esta semana
        self._create_metric(metrics_frame, "📆 Semana", str(self.stats.cases_this_week), 3)
    ...
    def _load_history_from_db(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT case_number, urgency, case_type, created_at FROM cases ORDER BY created_at DESC LIMIT 50"
            )
            ...
            if rows:
                ...
                for row in rows:
                    self._add_to_history(row[0], row[1], row[2] or "general", row[3])
        except Exception as e:
    ...
    def _add_to_history(self, case_number: str, urgency: str, category: str, timestamp=None):
