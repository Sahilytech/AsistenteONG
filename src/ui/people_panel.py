...
class PeoplePanel(ctk.CTkFrame):
 ...
 def _build(self):
  head=ctk.CTkFrame(self,fg_color="transparent");head.pack(fill="x",padx=24,pady=(22,10));head.grid_columnconfigure(0,weight=1);ctk.CTkLabel(head,text="Personas",font=FONTS["title"],text_color=COLORS["text"]).grid(row=0,column=0,sticky="w");ctk.CTkLabel(head,text="Un registro por persona. Los casos posteriores se agregan al mismo historial.",font=FONTS["small"],text_color=COLORS["text_muted"]).grid(row=1,column=0,sticky="w");ctk.CTkButton(head,text="＋ Nueva persona",width=130,height=36,command=self.new_person,fg_color=COLORS["primary"],hover_color=COLORS["primary_dark"]).grid(row=0,column=1,rowspan=2,padx=6);ctk.CTkButton(head,text="Importar XLSX / CSV / PDF",width=180,height=36,command=self.import_file,fg_color=COLORS["surface_alt"],hover_color=COLORS["primary_soft"],text_color=COLORS["text"],border_width=1,border_color=COLORS["border"]).grid(row=0,column=2,rowspan=2)
  self.search=ctk.CTkEntry(self,placeholder_text="Buscar por nombre, documento o contacto...",height=38);self.search.pack(fill="x",padx=24,pady=(0,10));self.search.bind("<KeyRelease>",lambda _:self.refresh());self.scroll=ctk.CTkScrollableFrame(self,fg_color="transparent");self.scroll.pack(fill="both",expand=True,padx=18,pady=(0,18))
 def refresh(self):
  for w in self.scroll.winfo_children():w.destroy()
  people=self.registry.list_people(self.search.get() if hasattr(self,"search") else "")
  if not people:
   empty=ctk.CTkFrame(self.scroll,fg_color=COLORS["surface_alt"],corner_radius=16,border_width=1,border_color=COLORS["border"]);empty.pack(fill="x",pady=25);ctk.CTkLabel(empty,text="No hay personas registradas.",font=FONTS["heading"],text_color=COLORS["text"]).pack(pady=(25,5));ctk.CTkLabel(empty,text="Podés crear una persona o importar registros existentes.",font=FONTS["small"],text_color=COLORS["text_muted"]).pack(pady=(0,25));return
  for p in people:self._person_card(p)
 def _person_card(self,p):
  card=ctk.CTkFrame(self.scroll,fg_color=COLORS["surface"],corner_radius=14,border_width=1,border_color=COLORS["border"]);card.pack(fill="x",padx=6,pady=5);card.grid_columnconfigure(1,weight=1);ctk.CTkLabel(card,text=p["name"],font=FONTS["subheading"],text_color=COLORS["text"],anchor="w").grid(row=0,column=0,sticky="w",padx=16,pady=(13,2));count=self.registry.case_count(p["person_id"]);meta=" · ".join(x for x in [p.get("birth_date"),p.get("document_id")] if x) or "Datos básicos disponibles";ctk.CTkLabel(card,text=f"{meta} · {count} caso(s)",font=FONTS["small"],text_color=COLORS["text_muted"]).grid(row=1,column=0,columnspan=2,sticky="w",padx=16,pady=(0,12));ctk.CTkButton(card,text="＋ Caso",width=72,height=31,command=lambda pid=p["person_id"]:self.new_case_for(pid),fg_color=COLORS["primary"],hover_color=COLORS["primary_dark"]).grid(row=0,column=2,rowspan=2,padx=4);ctk.CTkButton(card,text="Abrir expediente",width=125,height=31,command=lambda x=p:self.open_person(x),fg_color=COLORS["surface_alt"],hover_color=COLORS["primary_soft"],text_color=COLORS["primary"],border_width=1,border_color=COLORS["border"]).grid(row=0,column=3,rowspan=2,padx=(4,12))
 def new_case_for(self,pid):
  controller=getattr(self.winfo_toplevel(),"app_controller",None)
  if controller and "Casos" in controller.frames:controller.select_tab("Casos");controller.frames["Casos"].show_new_case(pid)
 def new_person(self):self._person_dialog({})
 def _person_dialog(self,data):
  win=ctk.CTkToplevel(self);win.title("Registro de persona");win.geometry("650x700");win.transient(self.winfo_toplevel());box=ctk.CTkScrollableFrame(win,fg_color="transparent");box.pack(fill="both",expand=True,padx=20,pady=20);fields=[("name","Nombre / alias"),("document_id","Documento"),("birth_date","Fecha de nacimiento"),("age","Edad"),("sex_at_birth","Sexo asignado al nacer"),("gender_identity","Identidad de género"),("sexual_orientation","Orientación sexual"),("contact","Contacto"),("address","Domicilio"),("notes","Notas")];entries={}
