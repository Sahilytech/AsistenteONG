...
class AboutPanel(ctk.CTkFrame):
    def __init__(self, parent, **kwargs):
        ...
        for title, text in [("PROPÓSITO", "Herramienta de apoyo para equipos de asistencia. Organiza información y tareas repetitivas sin reemplazar la revisión profesional."),("PERSONAS Y CASOS", "Una persona puede tener múltiples casos. El registro de persona se reutiliza y cada atención queda como un caso separado dentro de su historial."),("DOCUMENTACIÓN", "Los PDFs se procesan localmente y sus fragmentos relevantes pueden recuperarse al analizar un caso. Las coincidencias se muestran como apoyo documental, no como conclusiones."),("PRIVACIDAD", "El almacenamiento prioriza este equipo. Los campos personales sensibles son opcionales y la organización debe cargar únicamente lo necesario."),("LÍMITES", "El análisis no diagnostica ni decide por sí solo cuestiones legales, sanitarias o de protección. Requiere revisión humana.")]:
            card=ctk.CTkFrame(scroll, fg_color=COLORS["surface_alt"], corner_radius=16, border_width=1, border_color=COLORS["border"]); card.pack(fill="x", pady=6)
...
class MainWindow:
    def __init__(self):
        ...
        self.root.geometry("1500x900"); self.root.minsize(1080,700); self.root.configure(fg_color=COLORS["background"])
        self.root.grid_rowconfigure(0,weight=1); self.root.grid_columnconfigure(0,weight=1)
        self.frames={}; self.nav_buttons=[]; self._ui_ready=False; self._initializing=True
        self._setup_shell()
        # No construimos todos los paneles antes de que Tk pueda pintar.
        self.root.after_idle(self._initialize_services)

    def _setup_shell(self):
        self.loading_frame=ctk.CTkFrame(self.root,fg_color=COLORS["background"],corner_radius=0); self.loading_frame.grid(row=0,column=0,sticky="nsew")
        ...
        self.loading_bar=ctk.CTkProgressBar(self.loading_frame,width=360,mode="indeterminate",progress_color=COLORS["primary"]); self.loading_bar.place(relx=.5,rely=.57,anchor="center"); self.loading_bar.start()

    def _initialize_services(self):
        try:
            self.case_manager=CaseManager(); self.person_registry=PersonRegistry(); self.config_manager=ConfigManager(); self.memory=LocalMemory()
            self.session_guard=SessionGuard(timeout_seconds=900); self.session_guard.start(); self.session_verifier=None; self._lock_window=None
            self._setup_ui()
            self.root.app_controller=self
            self.root.bind_all("<Any-KeyPress>",self._activity_event,add="+"); self.root.bind_all("<Any-Button>",self._activity_event,add="+")
            self.root.bind("<Control-n>",lambda _e:self.open_new_case()); self.root.bind("<Control-b>",lambda _e:self.select_tab("Biblioteca")); self.root.bind("<Control-k>",lambda _e:self._focus_analysis()); self.root.bind("<Escape>",lambda _e:self._close_active_dialog())
            self.root.after(1000,self._maximize_after_start); self.root.after(1000,self._session_tick)
            self._ui_ready=True; self._initializing=False; self.loading_bar.stop(); self.loading_frame.destroy(); self.select_tab("Inicio")
            self.root.after(100,lambda:show_first_run(self.root))
        except Exception as exc:
            self._initializing=False; logger.error("Error inicializando la aplicación: %s",exc,exc_info=True)
            try:self.loading_bar.stop(); self.loading_frame.destroy(); ctk.CTkLabel(self.root,text="No se pudo iniciar el entorno",font=FONTS["title"],text_color=COLORS["text"]).pack(expand=True)
            except Exception:pass
            raise

    def _focus_analysis(self):
        self.select_tab("Análisis"); frame=self.frames.get("Análisis")
        for widget_name in ("search_entry","query_entry","input_box"):
            widget=getattr(frame,widget_name,None)
            if widget is not None:
                try:widget.focus_set();return
                except Exception:pass

    def _close_active_dialog(self):
        if self._lock_window is not None:
            try:self._lock_window.focus_set()
            except Exception:pass
    ...
    def _activity_event(self,_event=None):
        if not self.session_guard.locked:self.session_guard.touch()

    def _session_tick(self):
        try:
            if self.session_verifier and not self.session_guard.check():self.show_lock_screen()
        finally:self.root.after(1000,self._session_tick)

    def set_session_verifier(self,verifier):self.session_verifier=verifier;self.session_guard.start()

    def show_lock_screen(self):
        if self._lock_window is not None and self._lock_window.winfo_exists():self._lock_window.lift();return
        if not self.session_verifier:self.select_tab("Seguridad");return
        self.session_guard.lock(); win=ctk.CTkToplevel(self.root); self._lock_window=win; win.title("Sesión bloqueada"); win.geometry("620x380"); win.minsize(520,330); win.protocol("WM_DELETE_WINDOW",lambda:None); win.transient(self.root); win.grab_set()
        ctk.CTkLabel(win,text="Sesión bloqueada",font=FONTS["title"],text_color=COLORS["text"]).pack(pady=(55,8)); ctk.CTkLabel(win,text="La aplicación se bloqueó para proteger la información local.",font=FONTS["body"],text_color=COLORS["text_muted"],wraplength=470,justify="center").pack(pady=(0,22))
        entry=ctk.CTkEntry(win,placeholder_text="Frase de desbloqueo",show="•",width=360); entry.pack(pady=8); status=ctk.CTkLabel(win,text="",font=FONTS["tiny"],text_color=COLORS["text_muted"]); status.pack(pady=6)
        def unlock(_event=None):
            if self.session_guard.unlock(entry.get(),self.session_verifier):win.grab_release();win.destroy();self._lock_window=None
            else:status.configure(text="Frase incorrecta.");entry.delete(0,"end")
        ctk.CTkButton(win,text="Desbloquear",command=unlock,fg_color=COLORS["primary"],hover_color=COLORS["primary_dark"],width=180).pack(pady=16); entry.bind("<Return>",unlock); entry.focus_set()
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            if path.exists():
                im=Image.open(path).convert("RGBA");im.thumbnail((52,52));li=ctk.CTkImage(light_image=im,dark_image=im,size=(52,52));lab=ctk.CTkLabel(brand,image=li,text="");lab.image=li;lab.pack(side="left",padx=(10,8),pady=10)
        except Exception:pass
        ...
        for name,cls in specs:
            ...
            if "session_guard" in params:kwargs["session_guard"]=self.session_guard
            if "on_secret_configured" in params:kwargs["on_secret_configured"]=self.set_session_verifier
    ...
    def open_new_case(self):
        ...
        self.select_tab("Casos");self.frames["Casos"].show_new_case()
    ...
    def _on_case_submit(self,case_text,metadata):
        try:
            ...
            self.frames["Casos"].close_editor();self.frames["Análisis"].show_analysis(case.case_number,case_text,analysis);self.frames["Caso + Informe"].refresh_cases();self.select_tab("Análisis")
