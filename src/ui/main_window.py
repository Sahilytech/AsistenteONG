...
class MainWindow:
    ...
    def _setup_ui(self):
        ...
        tabs=["Inicio","Casos","Análisis","Seguimiento","Informe Social","Recursos","Biblioteca","Agenda","Seguridad","Configuración","Ayuda","Acerca de"]
        for target in tabs:
        ...
        workspace=ctk.CTkFrame(main,fg_color=COLORS["background"],corner_radius=0); workspace.grid(row=0,column=1,sticky="nsew",padx=(8,16),pady=14); workspace.grid_rowconfigure(0,weight=1); workspace.grid_columnconfigure(0,weight=1); self.workspace=workspace
    ...
    def _on_case_submit(self,case_text):
        try:
            ...
            casos=self.frames["Casos"]
            if hasattr(casos,"close_editor"): casos.close_editor()
            self.frames["Análisis"].show_analysis(case.case_number,case_text,analysis); self.select_tab("Análisis")
            try:self.frames["Inicio"].refresh(); self.frames["Casos"].refresh()
            except Exception:pass
