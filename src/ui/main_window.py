...
class AboutPanel(ctk.CTkFrame):
 def __init__(self,parent,**kwargs):
  super().__init__(parent,**kwargs); self.grid_rowconfigure(0,weight=1); self.grid_columnconfigure(0,weight=1); scroll=ctk.CTkScrollableFrame(self,fg_color="transparent"); scroll.grid(row=0,column=0,sticky="nsew",padx=28,pady=22)
  ctk.CTkLabel(scroll,text="Acerca de Asistente ONG",font=FONTS["title"],text_color=COLORS["text"]).pack(anchor="w"); ctk.CTkLabel(scroll,text="NubiWorks · tecnología local para apoyo social",font=FONTS["subheading"],text_color=COLORS["primary"]).pack(anchor="w",pady=(4,18))
  sections=[("EL PROYECTO","Asistente ONG es una suite de escritorio orientada a organizaciones sociales, fundaciones y equipos de asistencia. Ayuda a organizar relatos, realizar un triaje contextual orientativo, gestionar casos, preparar informes, consultar recursos y mantener seguimientos en un entorno local."),("EL PROBLEMA","Los equipos pequeños pueden recibir muchos mensajes y consultas mientras cuentan con tiempo y recursos limitados. El proyecto busca reducir tareas repetitivas de clasificación y organización sin convertir la automatización en una sustitución del criterio profesional."),("CÓMO FUNCIONA","El flujo comienza con el relato recibido. El motor local identifica contexto e indicadores, estima una prioridad orientativa, explica qué encontró, señala información faltante y sugiere recursos. Luego el profesional revisa, corrige cuando corresponda y decide la intervención."),("PRIVACIDAD Y OFFLINE","Asistente ONG está diseñado para funcionar sin conexión a Internet. El procesamiento, la base de datos, la memoria y los recursos utilizados por la aplicación permanecen en el equipo. No depende de APIs, buscadores, servicios cloud ni consultas de red para operar."),("QUÉ INCLUYE","Triaje contextual · gestión de casos · seguimiento · dashboard · recursos y filtros · informes sociales · biblioteca local · agenda · seguridad y copias · configuración institucional · búsqueda local · memoria local · ayuda integrada."),("QUÉ NO HACE","No diagnostica, no reemplaza profesionales, no confirma por sí solo una emergencia, no determina responsabilidades legales y no debe ser la única fuente para decisiones de alto impacto. Los resultados automáticos son apoyo para organizar información y deben revisarse."),("NUBIWORKS","NubiWorks es el proyecto y marca tecnológica detrás de esta herramienta. El objetivo es crear software accesible, privado y útil que pueda funcionar con recursos limitados y generar impacto social."),("SOBRE LA CREADORA","Sarah Lee Olivera es una estudiante y desarrolladora de software de Argentina interesada en tecnología con impacto social, aplicaciones, inteligencia artificial local, accesibilidad y herramientas que protejan la privacidad."),("TECNOLOGÍAS","Python · CustomTkinter · SQLite · procesamiento local · reglas explicables · generación de informes · almacenamiento local · arquitectura preparada para Windows.")]
  for title,text in sections:self._section(scroll,title,text)
  ctk.CTkFrame(scroll,height=1,fg_color=COLORS["border"]).pack(fill="x",pady=18); ctk.CTkLabel(scroll,text="Sarah Lee Olivera",font=FONTS["heading"],text_color=COLORS["primary"]).pack(anchor="w"); ctk.CTkLabel(scroll,text="Creadora y desarrolladora · NubiWorks",font=FONTS["small"],text_color=COLORS["text_muted"]).pack(anchor="w",pady=(3,15))
 def _section(self,parent,title,text):
  card=ctk.CTkFrame(parent,fg_color=COLORS["surface_alt"],corner_radius=16,border_width=1,border_color=COLORS["border"]); card.pack(fill="x",pady=6); ctk.CTkLabel(card,text=title,font=FONTS["subheading"],text_color=COLORS["primary"],anchor="w").pack(fill="x",padx=20,pady=(15,6)); body=ctk.CTkLabel(card,text=text,font=FONTS["body"],text_color=COLORS["text"],justify="left",anchor="w",wraplength=760); body.pack(fill="x",padx=20,pady=(0,18)); card.bind("<Configure>",lambda e,b=body:b.configure(wraplength=max(420,e.width-42)))
