...
class SplashScreen:
    ...
    def _draw_background(self):
        ...
        self.canvas.create_rectangle(m,m,right,bottom,outline=LIGHT,width=2); self.canvas.create_rectangle(m+16,m+16,right-16,bottom-16,outline="#F4FAFD",width=1)
        for y,direction in ((42,1),(bottom-40,-1)):
            ...
            for x in range(55,right-50,65):
                self.canvas.create_line(x,y,x+28*direction,y,fill=BLUE,width=1); node=self.canvas.create_oval(x-3,y-3,x+3,y+3,fill=LIGHT,outline=""); self._nodes.append(node)
        ...
        for x,sgn in ((55,1),(right-55,-1)):
            for i in range(4):
                y=170+i*75;end=x+sgn*55;self.canvas.create_line(x,y,end,y,fill=LIGHT,width=2);self.canvas.create_line(end,y,end,y+22,fill=BLUE,width=1)
        ...
        for i in range(16):
            p=self.canvas.create_oval(0,0,6,6,fill=BLUE,outline="");self._particles.append({"id":p,"x":55+(i*53)%(right-110),"y":100+(i*29)%(bottom-190),"dx":0.65 if i%2 else -0.5})
    def _load_logo(self):
    ...
    def start_loading(self):
        # Duración visual breve; la pantalla permanece visible mientras se construye la app real.
    ...
    def _finish(self):
        if self._closing:return
        try:
            # Construimos la ventana real antes de retirar el splash. Así nunca queda el escritorio visible.
            app=self.on_complete() if self.on_complete else None
            if app is not None and hasattr(app,"root"):
                app.root.deiconify();app.root.update_idletasks();app.root.lift();app.root.focus_force()
                self._closing=True
