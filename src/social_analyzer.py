...
class SocialReportAnalyzer:
    REQUIRED = {
        ...
        "egresos": "Egresos básicos",
        "tenencia": "Régimen de tenencia",
        "condiciones_vivienda": "Condiciones de vivienda",
        "servicios_entorno": "Servicios y entorno",
        "salud": "Situación sanitaria",
        "educacion": "Educación",
        "diagnostico": "Valoración social / juicio técnico",
        "propuesta": "Propuesta de intervención",
    }
    ...
    def analyze(self, data):
        ...
        overcrowding = None
        if household and rooms:
            overcrowding = round(household / rooms, 2)

        income = self._money_total(data.get("ingresos", ""))
        expenses = self._money_total(data.get("egresos", ""))
        ...
        if birth:
            today = date.today()
            derived["edad_calculada"] = today.year - birth.year - ((today.month, today.day) < (birth.month, birth.day))
        ...
        if balance is not None and balance < 0:
            consistency.append("Los egresos declarados superan los ingresos declarados.")
        if overcrowding is not None and overcrowding > 2:
            consistency.append("La relación declarada entre convivientes y habitaciones requiere revisión por posible hacinamiento.")

        if indicators:
            level = "Revisión prioritaria"
        elif missing:
            level = "Información incompleta"
        else:
            level = "Sin indicadores automáticos destacados"

        strengths = self._strengths(data)
        return {
            "level": level,
            "completeness": round(100 * (len(self.REQUIRED) - len(missing)) / len(self.REQUIRED)),
            "missing_fields": missing,
            "risk_indicators": indicators,
            "strengths": strengths,
            "household_members": household,
            "rooms": rooms,
            "persons_per_room": overcrowding,
            "income_total": income,
            "expenses_total": expenses,
            "balance": balance,
            "derived": derived,
            "consistency_flags": consistency,
            "recommendations": self._recommendations(indicators, missing, consistency),
    ...
    def _recommendations(self, indicators, missing, consistency):
        out = []
        if missing:
            out.append("Completar los campos faltantes antes de emitir el informe.")
        if indicators:
            out.append("Revisar manualmente los indicadores detectados y confirmar contexto, temporalidad y nivel de riesgo.")
        if "precariedad económica" in indicators:
            out.append("Verificar ingresos, prestaciones disponibles, gastos esenciales y recursos de apoyo económico.")
        if "precariedad habitacional" in indicators:
            out.append("Verificar condiciones de habitabilidad, servicios básicos y alternativas habitacionales.")
        if "niñez o adolescencia" in indicators:
            out.append("Revisar necesidades de protección y situación educativa de niños, niñas o adolescentes involucrados.")
        if "violencia" in indicators or "violencia sexual" in indicators:
            out.append("Priorizar una entrevista segura y valorar necesidades de protección, acompañamiento y derivación especializada.")
        if consistency:
            out.append("Resolver las inconsistencias señaladas antes de cerrar el informe.")
        if not out:
            out.append("Mantener revisión profesional y actualizar el informe si cambia la situación social.")
        return out

    def _strengths(self, data):
        text = " ".join(str(data.get(k, "")) for k in ("dinamica_familiar", "fortalezas", "situacion_laboral", "educacion", "servicios_entorno")).lower()
        labels = []
    ...
    def _money_total(value):
        ...
        for whole, cents in matches:
            try:
                total += float(whole.replace(".", "") + ("." + cents if cents else ""))
            except ValueError:
                pass
