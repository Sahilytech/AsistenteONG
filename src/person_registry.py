...
class PersonRegistry:
 ...
 def link_case(self,case_number,pid):
  with self._db() as db:db.execute("UPDATE cases SET person_id=? WHERE case_number=?",(pid,case_number))
 def cases(self,pid):
...
class PersonImporter:
 ...
 def map_row(self,row):
  ...
  for field,aliases in FIELDS.items():
   for alias in aliases:
    if self._key(alias) in normalized:mapped[field]=normalized[self._key(alias)];break
  return mapped
 ...
 def import_xlsx(self,path):
  ...
  ws=load_workbook(path,read_only=True,data_only=True).active;rows=list(ws.iter_rows(values_only=True))
  if not rows:return 0,0
  headers=[str(x or "") for x in rows[0]];return self._import_rows(dict(zip(headers,v)) for v in rows[1:])
 def import_csv(self,path):
  with open(path,"r",encoding="utf-8-sig",newline="") as f:return self._import_rows(csv.DictReader(f))
 def import_pdf(self,path):
  from pypdf import PdfReader
  text="\n".join((p.extract_text() or "") for p in PdfReader(path).pages);records=[]
  ...
  return self._import_rows(records)
