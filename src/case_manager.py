...
class CaseManager:
    def __init__(self, db_path: str = None):
        self.db_path = db_path or str(DB_PATH); self._init_db(); logger.info("CaseManager inicializado en %s", self.db_path)
    def _init_db(self):
        conn=sqlite3.connect(self.db_path); cur=conn.cursor()
        cur.execute("""CREATE TABLE IF NOT EXISTS cases(case_id TEXT PRIMARY KEY,case_number TEXT UNIQUE,created_at TEXT,text TEXT,urgency TEXT,keywords TEXT,notes TEXT,status TEXT,assigned_to TEXT,resources_used TEXT,follow_up_date TEXT,person_name TEXT,contact TEXT,case_type TEXT,location TEXT,social_report TEXT,combined_analysis TEXT,timeline TEXT,referrals TEXT)""")
        existing={r[1] for r in cur.execute("PRAGMA table_info(cases)").fetchall()}
        additions={"person_name":"TEXT DEFAULT ''","contact":"TEXT DEFAULT ''","case_type":"TEXT DEFAULT ''","location":"TEXT DEFAULT ''","social_report":"TEXT DEFAULT '{}'","combined_analysis":"TEXT DEFAULT '{}'","timeline":"TEXT DEFAULT '[]'","referrals":"TEXT DEFAULT '[]'"}
        for name,definition in additions.items():
            if name not in existing: cur.execute(f"ALTER TABLE cases ADD COLUMN {name} {definition}")
    ...
    def create_case(self,text:str,urgency:str="no determinada",keywords:Optional[List[str]]=None,metadata:Optional[Dict]=None,analysis:Optional[Dict]=None)->Case:
        metadata=metadata or {}; profile=build_case_profile(text,metadata); brain=analysis or analyze_profile(profile)
        detected_keywords=keywords or sorted(set(profile.indicators+profile.relationships+profile.contexts))
        case=Case(str(uuid.uuid4())[:8],self.generate_case_number(),datetime.now().isoformat(),text,brain.get("urgency",urgency),detected_keywords,person_name=metadata.get("person_name","").strip(),contact=metadata.get("contact","").strip(),case_type=metadata.get("case_type","").strip(),location=metadata.get("location","").strip(),combined_analysis={**brain,"profile":profile.to_dict()})
        case.timeline.append({"event_type":"caso","title":"Caso creado","description":"Registro inicial del caso","created_at":datetime.now().isoformat(timespec="seconds"),"actor":"","status":"registrado"})
        self.save_case(case); logger.info("Caso creado: %s",case.case_number); return case
    def save_case(self,case:Case):
        conn=sqlite3.connect(self.db_path); cur=conn.cursor(); cur.execute("""INSERT OR REPLACE INTO cases VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(case.case_id,case.case_number,case.created_at,case.text,case.urgency,json.dumps(case.keywords,ensure_ascii=False),case.notes,case.status,case.assigned_to,json.dumps(case.resources_used,ensure_ascii=False),case.follow_up_date,case.person_name,case.contact,case.case_type,case.location,json.dumps(case.social_report,ensure_ascii=False),json.dumps(case.combined_analysis,ensure_ascii=False),json.dumps(case.timeline,ensure_ascii=False),json.dumps(case.referrals,ensure_ascii=False))); conn.commit(); conn.close()
    def attach_social_report(self,case_number:str,report:Dict,analysis:Optional[Dict]=None):
        ...
        if analysis:
            case.combined_analysis=analysis; case.urgency=analysis.get("urgency",case.urgency); case.keywords=analysis.get("keywords",case.keywords); case.resources_used=analysis.get("suggested_resources",case.resources_used)
        case.timeline.append({"event_type":"informe","title":"Informe social actualizado","description":"El informe fue asociado al análisis integral","created_at":datetime.now().isoformat(timespec="seconds"),"actor":"","status":"registrado"})
    ...
    def add_referral(self,case_number:str,referral:Dict):
        case=self.get_case(case_number)
        if not case: raise ValueError(f"No existe el caso {case_number}")
        item=dict(referral); item.setdefault("status","pendiente"); item.setdefault("created_at",datetime.now().isoformat(timespec="seconds")); case.referrals.append(item); case.timeline.append({"event_type":"derivacion","title":"Derivación registrada","description":item.get("name", ""),"created_at":datetime.now().isoformat(timespec="seconds"),"actor":"","status":"registrado"}); self.save_case(case); return case
